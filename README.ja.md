...
Claude Code で `batch_design` を実行するたびに、デザインが自動的にPDFエクスポートされ、cmuxブラウザペインに反映されます。
...
PostToolUse hook が自動登録されます。プレビューを開始:
...
batch_design → PostToolUse hook 発火
  ...
  → Claude が export_nodes (PDF) を実行
  → Claude が mv + cmux browser reload を実行
```
